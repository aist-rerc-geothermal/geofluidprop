...
{
    return driesner07_H2O_NaCl_pc_T2(T, false);
}
...
double driesner07_H2O_NaCl_Tc_x(double x)
{
    ...
    for (int i=0; i<100; i++)
    {
        double x1 = driesner07_H2O_NaCl_xc_T(T);
        double r = x1 - x;
        //printf("%d: T=%g, x=%g, r=%g\n", i, T, x1, r);
        if (fabs(r) < 1e-15)
            return T;
        double drdT = (driesner07_H2O_NaCl_xc_T(T+d) - x1)/d;
        T += -r/drdT;
...
double driesner07_H2O_NaCl_VL_log10_Kn(double pn, double T_C)
{
...
double driesner07_H2O_NaCl_VL_log10_Km(double p_bar, double T_K)
{
    double T_C = to_C(T_K);
    double p_NaCl = to_bar(driesner07_NaCl_VH_p_T(T_K));
    double pc = to_bar(driesner07_H2O_NaCl_pc_T(T_K));
    double pn = (p_bar - p_NaCl)/(pc - p_NaCl);
    double log10Kn = driesner07_H2O_NaCl_VL_log10_Kn(pn, T_C);
    double x_sat_l = driesner07_H2O_NaCl_LH_xl_pT(to_Pa(p_NaCl), T_K);
...
// K from Eqs. (14)
double driesner07_H2O_NaCl_VL_K(double p, double T)
{
    double p_bar = to_bar(p);
    double p_NaCl = to_bar(driesner07_NaCl_VH_p_T(T));
    double log_Km = driesner07_H2O_NaCl_VL_log10_Km(p_bar, T);
    double log_K = log_Km - log10(p_NaCl/p_bar);
    return pow(10., log_K);
}


double driesner07_H2O_vm_pT(double p, double T)
{
...
double driesner07_H2O_NaCl_VL_xl_pT(double p_Pa, double T_K)
{
    ...
    double const pc = to_bar(driesner07_H2O_NaCl_pc_T(T_K));
    double xc = driesner07_H2O_NaCl_xc_T(T_K);
    ...
    if (T_K < driesner07_NaCl_VLH_T())
    {
        ...
        double p_VLH = driesner07_H2O_NaCl_VLH_p_T(T_K);
        double x_VLH = driesner07_H2O_NaCl_LH_xl_pT(p_VLH, T_K);
        ...
        double x_NaCl_boil = 1.0; //driesner07_H2O_NaCl_LH_xl_pT(p_NaCl_boil, T_K);
        //printf("x_LH,sat=%e\n", driesner07_H2O_NaCl_LH_xl_pT(p_NaCl_boil, T_K));
...
{
    double x = driesner07_H2O_NaCl_VL_xl_pT(p,T)/driesner07_H2O_NaCl_VL_K(p,T);
    return x;
}
...
#pragma region LH
// H2O(l)-NaCl(s)
double driesner07_H2O_NaCl_LH_xl_pT(double p_Pa, double T_K)
{
...
{
    double x_v_sat = driesner07_H2O_NaCl_LH_xl_pT(p, T)
...
// H2O(v)-H2O(l)-NaCl(s)
double driesner07_H2O_NaCl_VLH_p_T(double T_K)
{
...
{
    double p = driesner07_H2O_NaCl_VLH_p_T(T);
...
static bool isSuperCritical(double p, double T, double x)
{
    ...
    const double xc = driesner07_H2O_NaCl_xc_T(T);
    const double pc = driesner07_H2O_NaCl_pc_T(T);
    return (p >= pc); // && x >= xc);
}
...
static bool isVapor(double p, double T, double x)
{
    ...
    if (T >= H2O_Tc && p >= driesner07_H2O_NaCl_pc_T(T))
        return false;
    ...
    if (p < pVLH)
        return true;
//        xv_max  = driesner07_H2O_NaCl_VH_xv_pT(p, T);
    // p in VL
    xv_max = driesner07_H2O_NaCl_VL_xv_pT(p, T);
    return (x <= xv_max);
}
...
static bool isLquid(double p, double T, double x)
{
    ...
    // check xmin
    double x_min = driesner07_H2O_NaCl_VL_xl_pT(p, T);
    return (x_min <= x);
}
...
static bool isTwoPhase(double p, double T, double x)
{
    ...
    double VL_x_min = driesner07_H2O_NaCl_VL_xv_pT(p, T);
    double VL_x_max = driesner07_H2O_NaCl_VL_xl_pT(p, T);
...
H2O_NaCl_PhaseType driesner07_H2O_NaCl_phase_type(double p, double T, double x)
{
    ...
    if (isLquid(p, T, x))
    {
        double xsat = driesner07_H2O_NaCl_NaCl_solubility_xl_pT(p,T);
    ...
    if (isTwoPhase(p, T, x))
    {
        if (p > driesner07_H2O_NaCl_VLH_p_T(T))
            return H2O_NaCl_PhaseType_VL;
        return H2O_NaCl_PhaseType_VLH;
    }

    if (isVapor(p, T, x)) {
        if (x < driesner07_H2O_NaCl_NaCl_solubility_xv_pT(p,T))
            return H2O_NaCl_PhaseType_V;
        return H2O_NaCl_PhaseType_VH;
    }
    if (isSuperCritical(p, T, x))
    {
        if (x < driesner07_H2O_NaCl_NaCl_solubility_xl_pT(p,T))
            return H2O_NaCl_PhaseType_SC;
...
char* driesner07_H2O_NaCl_phase_type_to_str(H2O_NaCl_PhaseType phase, char*name)
{
...
double driesner07_H2O_NaCl_Tv_pTx(double p /*bar*/, double T /*C*/, double x)
{
...
double driesner07_H2O_NaCl_vm_pTx0(double p_Pa, double T_K, double x)
{
    const double p = to_bar(p_Pa);
    const double T = to_C(T_K);

    double Tv = driesner07_H2O_NaCl_Tv_pTx(p, T, x);
    double Tv_K = to_K(Tv);
    //printf("Tv=%g\n",Tv);
    assert(Tv>0);
...
double driesner07_H2O_NaCl_vm_pTx1(double p_Pa, double T_K, double x)
{
    ...
    // compute V at Tv
    double Tv = driesner07_H2O_NaCl_Tv_pTx(p_bar, T_C, x);
...
double driesner07_H2O_NaCl_vm_pTx2(double p_Pa, double T_K, double x)
{
    const double p = to_bar(p_Pa);
    const double T = to_C(T_K);
    ...
    double Vm_p1 = driesner07_H2O_NaCl_vm_pTx0(to_Pa(p1), T_K, x);
    double dVm_p1 = driesner07_H2O_NaCl_vm_pTx0(to_Pa(p1+dp), T_K, x);
    dVm_p1 -= driesner07_H2O_NaCl_vm_pTx0(to_Pa(p1-dp), T_K, x);
    dVm_p1 /= 2.*dp;
    double dVm_p2 = driesner07_H2O_NaCl_vm_pTx0(to_Pa(p2+dp), T_K, x);
    dVm_p2 -= driesner07_H2O_NaCl_vm_pTx0(to_Pa(p2-dp), T_K, x);
    dVm_p2 /= 2.*dp;
...
double driesner07_H2O_NaCl_vm_singlephase_pTx(double p_Pa, double T_K, double x)
{
    ...
    const double Tv_C = driesner07_H2O_NaCl_Tv_pTx(p_bar, T_C, x);
    assert(Tv_C>0);
    const double Tv_K = to_K(Tv_C);
    //printf("Tv=%g\n",Tv);

    H2O_NaCl_PhaseType phase_id = driesner07_H2O_NaCl_phase_type(p_Pa, T_K,  x);
    double v = -1;
    if (phase_id/10 == H2O_NaCl_PhaseType_L/10 && p_Pa < 20e5)
    {
        if (Tv_K > h2o_sat_T(p_Pa)) // in case Tv exceeds boiling temperature
            v = driesner07_H2O_NaCl_vm_pTx1(p_Pa, T_K, x);
    } else if (phase_id/10 == H2O_NaCl_PhaseType_L/10 && T_C > 600 && p_Pa < 40e6) {
        // at high T (> ca600C), low p (< ca35 MPa), high xNaCl (> xVL,liquid)
        double VL_xl = driesner07_H2O_NaCl_VL_xl_pT(p_Pa, T_K);
        if (x >= VL_xl)
            v = driesner07_H2O_NaCl_vm_pTx2(p_Pa, T_K, x);
    }

    // default case
    if (v < 0)
        v = driesner07_H2O_NaCl_vm_pTx0(p_Pa, T_K, x);
...
{
    double mv = driesner07_H2O_NaCl_vm_singlephase_pTx(p_Pa, T_K, x); //m3/mol
...
{
    double xv = driesner07_H2O_NaCl_VL_xv_pT(p_Pa, T_K);
    double mv = driesner07_H2O_NaCl_vm_singlephase_pTx(p_Pa, T_K, xv); //m3/mol
...
{
    double x = driesner07_H2O_NaCl_VL_xl_pT(p_Pa, T_K);
    double molar_v = driesner07_H2O_NaCl_vm_singlephase_pTx(p_Pa, T_K, x); //m3/mol
    double total_mol = 1./(x*NaCl_molar_mass+(1.-x)*H2O_molar_mass); // mol/kg
    double rho = 1./total_mol * 1./molar_v;
    return rho;
}

double driesner07_H2O_NaCl_rhol_pTx1(double p_Pa, double T_K, double x)
{
    double mv = driesner07_H2O_NaCl_vm_pTx1(p_Pa, T_K, x); //m3/mol
    double total_mol = 1./(x*NaCl_molar_mass+(1-x)*H2O_molar_mass); // mol/kg
    double rho = 1./total_mol * 1./mv;
    return rho;
}

double driesner07_H2O_NaCl_rhol_pTx2(double p_Pa, double T_K, double x)
{
    double mv = driesner07_H2O_NaCl_vm_pTx2(p_Pa, T_K, x); //m3/mol
    double total_mol = 1./(x*NaCl_molar_mass+(1-x)*H2O_molar_mass); // mol/kg
    double rho = 1./total_mol * 1./mv;
    return rho;
}
...
double driesner07_H2O_NaCl_NaCl_solubility_xv_pT(double p, double T)
{
    double pVLH = driesner07_H2O_NaCl_VLH_p_T(T);
    if (p<=pVLH)
        return driesner07_H2O_NaCl_VH_xv_pT(p, T);
...
double driesner07_H2O_NaCl_NaCl_solubility_xl_pT(double p, double T)
{
    ...
    return driesner07_H2O_NaCl_LH_xl_pT(p, T);
}
...
double H2O_NaCl_Th(double p /*bar*/, double TC /*C*/, double x)
{
...
double driesner07_H2O_NaCl_mass_fraction_liquid(double p_Pa, double T_K, double x)
{
    ...
    {
        double xl = driesner07_H2O_NaCl_VL_xl_pT(p_Pa, T_K);
        double xv = driesner07_H2O_NaCl_VL_xv_pT(p_Pa, T_K);
...
{
    double ml = driesner07_H2O_NaCl_mass_fraction_liquid(p_Pa, T_K, x);
    return (1.-ml);
}
...
double driesner07_H2O_NaCl_volume_fraction_liquid(double p_Pa, double T_K, double x)
{
    if (isTwoPhase(p_Pa, T_K, x))
    {
        double xl = driesner07_H2O_NaCl_VL_xl_pT(p_Pa, T_K);
        double xv = driesner07_H2O_NaCl_VL_xv_pT(p_Pa, T_K);
        double rhol = driesner07_H2O_NaCl_VL_rhol_pT(p_Pa, T_K);
        double rhov = driesner07_H2O_NaCl_VL_rhov_pT(p_Pa, T_K);
...
{
    double satl = driesner07_H2O_NaCl_volume_fraction_liquid(p_Pa, T_K, x);
    return (1.-satl);
}
...
double driesner07_H2O_NaCl_volume_fraction_halite(double p_Pa, double T_K, double x)
{
    ...
    H2O_NaCl_PhaseType phase_type = driesner07_H2O_NaCl_phase_type(p_Pa, T_K, x);
    ...
    if (phase_type==H2O_NaCl_PhaseType_VH)
    {
        double xv = driesner07_H2O_NaCl_VL_xv_pT(p_Pa, T_K);
        double xh = 1.;
        double rhov = driesner07_H2O_NaCl_VL_rhol_pT(p_Pa, T_K);
    ...
    double xl = driesner07_H2O_NaCl_VL_xl_pT(p_Pa, T_K);
    double xh = 1.;
    double rhol = driesner07_H2O_NaCl_VL_rhov_pT(p_Pa, T_K);
    double rhoh = driesner07_NaCl_H_rho_pT(p_Pa, T_K);
    double sath = rhol*(xl-x)/(rhoh*(x-xh)+rhol*(xl-x));
    return sath;
}

double driesner07_H2O_NaCl_T_rhopx(double rho, double p_Pa, double x, double T0)
{
    double T_K = T0;
    ...
    {
        double rho_tmp = driesner07_H2O_NaCl_rho_pTx(p_Pa, T_K, x);
        ...
        double dT = 1e-3;
        double drhodT = (driesner07_H2O_NaCl_rho_pTx(p_Pa, T_K+dT, x) - rho_tmp)/dT;
...
double driesner07_H2O_NaCl_volume_fraction_liquid_rhopx(double rho, double p_Pa, double x, double T0)
{
    double T_K = driesner07_H2O_NaCl_T_rhopx(rho, p_Pa, x, T0);
    double satl = driesner07_H2O_NaCl_volume_fraction_liquid(p_Pa, T_K, x);
    return satl;
}

double driesner07_H2O_NaCl_T_phx(double p_Pa, double h, double x, double T0)
{
    double T_K = T0;
    const double rtol = 1e-6;
    const int nitrmax = 50;
    for (int i=0; i<nitrmax; i++)
    {
        double h_tmp = driesner07_H2O_NaCl_h_pTx(p_Pa, T_K, x);
        double r_h = h_tmp - h;
#ifdef DEBUG_INVERSE
        ...
        double dT = 1e-3;
        double drdT = (driesner07_H2O_NaCl_h_pTx(p_Pa, T_K+dT, x) - h_tmp)/dT;
...
double driesner07_H2O_NaCl_rho_pTx(double p_Pa, double T_K, double x)
{
    ...
    {
        return driesner07_H2O_NaCl_rho_singlephase_pTx(p_Pa, T_K, x);
    }
             ...
             || phase_type == H2O_NaCl_PhaseType_VLH)
    {
        ...
        double xv = driesner07_H2O_NaCl_VH_xv_pT(p_Pa, T_K);
        double rhov = driesner07_H2O_NaCl_rho_singlephase_pTx(p_Pa, T_K, xv);
        return rhov; //TODO halite precipitation
    } else if (phase_type == H2O_NaCl_PhaseType_LH
               || phase_type == H2O_NaCl_PhaseType_SCH) {
        double xl = driesner07_H2O_NaCl_LH_xl_pT(p_Pa, T_K);
        double rhol = driesner07_H2O_NaCl_rho_singlephase_pTx(p_Pa, T_K, xl);
...
double driesner07_H2O_NaCl_h_pTx(double p_Pa, double T_K, double x)
{
    ...
    else if (phase_type == H2O_NaCl_PhaseType_VL || phase_type == H2O_NaCl_PhaseType_VLH)
    {
        double xl = driesner07_H2O_NaCl_VL_xl_pT(p_Pa, T_K);
        double rhol = driesner07_H2O_NaCl_VL_rhol_pT(p_Pa, T_K);
        double hl = driesner07_H2O_NaCl_h_pTx(p_Pa, T_K, xl);
        double xv = driesner07_H2O_NaCl_VL_xv_pT(p_Pa, T_K);
        double rhov = driesner07_H2O_NaCl_VL_rhov_pT(p_Pa, T_K);
        double hv = driesner07_H2O_NaCl_h_pTx(p_Pa, T_K, xv);
        double satl = driesner07_H2O_NaCl_volume_fraction_liquid(p_Pa, T_K, x);
        double rho = satl*rhol + (1.-satl)*rhov;
        ...
        double xv = driesner07_H2O_NaCl_VH_xv_pT(p_Pa, T_K);
        double rhov = driesner07_H2O_NaCl_rho_singlephase_pTx(p_Pa, T_K, xv);
        double hv = driesner07_H2O_NaCl_h_singlephase_pTx(p_Pa, T_K, xv);
        return hv; //TODO halite precipitation
    } else if (phase_type == H2O_NaCl_PhaseType_LH) {
        double xl = driesner07_H2O_NaCl_LH_xl_pT(p_Pa, T_K);
        double rhol = driesner07_H2O_NaCl_rho_singlephase_pTx(p_Pa, T_K, xl);
        double hl = driesner07_H2O_NaCl_h_singlephase_pTx(p_Pa, T_K, xl);
...
double driesner07_H2O_NaCl_VL_p_Txv(double T_K, double x, double p0)
{
    ...
    double pVLH = 0.1;
    if (T_K < driesner07_NaCl_VLH_T())
        pVLH = driesner07_H2O_NaCl_VLH_p_T(T_K);
    double pc = driesner07_H2O_NaCl_pc_T(T_K);
    p = max(pVLH, min(p, pc));
    const double rtol = 1e-6;
    for (int i=0; i<100; i++)
    {
        double x_tmp = driesner07_H2O_NaCl_VL_xv_pT(p, T_K);
        double r = x_tmp - x;
        ...
        double drdp = 0;
        if (p+dp > pc)
            drdp = (x_tmp - driesner07_H2O_NaCl_VL_xv_pT(p-dp, T_K))/dp;
        else
            drdp = (driesner07_H2O_NaCl_VL_xv_pT(p+dp, T_K) - x_tmp)/dp;
...
int driesner07_H2O_NaCl_VL_p_all_Txv(double T_K, double x, double pmin, double pmax, double* p)
{
    double p1 = driesner07_H2O_NaCl_VL_p_Txv(T_K, x, pmin*1.01);
    double p2 = driesner07_H2O_NaCl_VL_p_Txv(T_K, x, pmax*0.99);
    if (p1 < 0 && p2 < 0) {
        p1 = driesner07_H2O_NaCl_VL_p_Txv(T_K, x, (pmin+pmax)*0.5);
        if (p1 < 0) return 0;
    }
    if (p2 < 0) p2 = p1;
    if (p1 < 0) p1 = p2;
...
double driesner07_H2O_NaCl_VL_p_Txl(double T_K, double x, double p0)
{
    ...
    for (int i=0; i<50; i++)
    {
        double x_tmp = driesner07_H2O_NaCl_VL_xl_pT(p, T_K);
        double r = x_tmp - x;
        ...
        double drdp = 0;
        if (p+dp > pc)
            drdp = (x_tmp - driesner07_H2O_NaCl_VL_xl_pT(p-dp, T_K))/dp;
        else
            drdp = (driesner07_H2O_NaCl_VL_xl_pT(p+dp, T_K) - x_tmp)/dp;
        double deltap = -r / drdp;
        p += deltap;
        if (p < pVLH) p = pVLH;
        if (p > pc) p = pc;
...
int driesner07_H2O_NaCl_sat_p_Tx(double T_K, double x, double p0, double* p)
{
    ...
    {
        double pv[2];
        int nv = driesner07_H2O_NaCl_VL_p_all_Txv(T_K, x, 0.1, 250e6, pv);
        for (int i=0; i<nv; i++)
            p[n++] = pv[i];
        double pl = driesner07_H2O_NaCl_VL_p_Txl(T_K, x, p0);
        if (pl > 0)
            p[n++] = pl;
        return n;
    }
    // below melting point of NaCl
    const double xlVLH = driesner07_H2O_NaCl_VLH_xl_T(T_K);
    const double pVLH = driesner07_H2O_NaCl_VLH_p_T(T_K);
    if (x >= xlVLH)
    {
        p[n++] = pVLH;
        return n;
    ...
    const double xc = driesner07_H2O_NaCl_xc_T(T_K);
    const double xvVLH = driesner07_H2O_NaCl_VLH_xv_T(T_K);
    const double pc = driesner07_H2O_NaCl_pc_T(T_K);
    if (x < xvVLH)
    {
        double pv[2];
        int nv = driesner07_H2O_NaCl_VL_p_all_Txv(T_K, x, pVLH, pc, pv);
        if (xc < x)
        {
            assert(nv==1);
            p[n++] = pv[0];
            p[n++] = driesner07_H2O_NaCl_VL_p_Txl(T_K, x, p0);
            return n;
        }
        for (int i=0; i<nv; i++)
            p[n++] = pv[i];
    ...
    if (x > xvVLH && xc > xvVLH)
    {
        double pVLH = driesner07_H2O_NaCl_VLH_p_T(T_K);
        double pl = driesner07_H2O_NaCl_VL_p_Txl(T_K, x, p0);
        double pv[2] = {0, 0};
        int nv = driesner07_H2O_NaCl_VL_p_all_Txv(T_K, x, pVLH, pc, pv);
        if (nv==0)
...
// double driesner07_H2O_NaCl_sat_p_upper_Tx(double T_K, double x, double p0)
// {
//     if (T_K > driesner07_NaCl_VLH_T())
//         return -1;
//     if (x==0)
...
//     double pv = driesner07_H2O_NaCl_VL_p_Txv(T_K, x, p0);
//     double pl = driesner07_H2O_NaCl_VL_p_Txl(T_K, x, p0);
//     // if (pv > 0 && pl > 0)
//     //     return min(pv, pl);
...
// double driesner07_H2O_NaCl_sat_p_lower_Tx(double T_K, double x, double p0)
// {
//     if (T_K > driesner07_NaCl_VLH_T())
//         return -1;
//     if (x==0)
...
//     double xvVLH = driesner07_H2O_NaCl_VLH_xv_T(T_K);
